...
class ZkBridge(Help):
    def __init__(self, privatekey, delay, chain: Chain, to_chain, api, nft, proxy=None):
        ...
        self.moralisapi = api
        self.nft_address = nfts_addresses['ZkBridge on opBNB'][Chain.BSC]
        #self.bridge_address = nft_lz_bridge_addresses[self.chain] if self.nft == 'Pandra' and self.to_chain != 'combo' else nft_bridge_addresses[self.chain]
    ...
    async def mint(self):
        while True:
            ...
            try:
                ...
                logger.info(f'{self.address}:{chain_name} - начинаю минт {self.nft}...')
                ...
                if status == 1:
                    logger.success(
                        f'{self.address}:{chain_name} - успешно заминтил {self.nft} : {scan}{self.w3.to_hex(hash)}...')
                    await self.sleep_indicator(random.randint(self.delay[0], self.delay[1]))
                    return headers
                else:
                    logger.info(f'{self.address}:{chain_name} - пробую минт еще раз...')
                    await self.mint()
            except Exception as e:
                error = str(e)
                if 'nonce too low' in error or 'already known' in error:
                    logger.success(f'{self.address}:{chain_name} - ошибка при минте, пробую еще раз...')
                    await asyncio.sleep(10)
                    await self.mint()
                if 'INTERNAL_ERROR: insufficient funds' in error or 'insufficient funds for gas * price + value' in error:
                    logger.error(
                        f'{self.address}:{chain_name} - не хватает денег на газ, заканчиваю работу через 5 секунд...')
                    await asyncio.sleep(5)
                    return False
                elif 'Each address may claim one NFT only. You have claimed already' in error:
                    logger.error(f'{self.address}:{chain_name} - {self.nft} можно клеймить только один раз!...')
                    return False
                else:
                    logger.error(f'{self.address}:{chain_name} - {e}...')
                    return False
            ...
            while True:
                if id_:
                    try:
                        ...
                        if status == 1:
                            logger.success(
                                f'{self.address}:{chain_name} - успешно апрувнул {self.nft} {id_} : {scan}{self.w3.to_hex(hash)}...')
                            await self.sleep_indicator(random.randint(1, 10))
                            return True
                        else:
                            logger.info(f'{self.address}:{chain_name} - пробую апрув еще раз...')
                            await approve_nft()
                    except Exception as e:
                        error = str(e)
                        if 'nonce too low' in error or 'already known' in error:
                            logger.info(f'{self.address}:{chain_name} - ошибка при апруве, пробую еще раз...')
                            await approve_nft()
                        if 'INTERNAL_ERROR: insufficient funds' in error or 'insufficient funds for gas * price + value' in error:
                            logger.error(
                                f'{self.address}:{chain_name} - не хватает денег на газ, заканчиваю работу через 5 секунд...')
            ...
            while True:
                try:
                    if self.nft == 'Pandra' and self.to != 'combo':
                        ...
                        to_chain = DATA[self.to_chain] #chain_ids[self.to]
                    ...
                    if status == 1:
                        ...
                        return self.privatekey, self.address, f'successfully bridged {self.nft} to {self.to_chain}'
                    else:
